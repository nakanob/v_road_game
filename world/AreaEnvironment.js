...
const PRESETS = [
  ...
  { bg:0xd47855, fogNear:165, fogFar:520, hemiSky:0xf2b483, hemiGround:0x51433b, hemi:1.28, sun:0xff9e62, intensity:2.25, pos:[70,45,20], exposure:.95, stars:.08 },
...
export class AreaEnvironment {
  ...
  update(progress, delta) {
    ...
    this.mix = Math.min(1, this.mix + delta * .26);
    ...
    this.game.ambient.color.lerpColors(this.from.hemiSky, this.target.hemiSky, k);
    this.game.ambient.groundColor.lerpColors(this.from.hemiGround, this.target.hemiGround, k);
    this.game.ambient.intensity = THREE.MathUtils.lerp(this.from.hemi, this.target.hemi, k);

    this.game.sun.color.lerpColors(this.from.sun, this.target.sun, k);
    this.game.sun.intensity = THREE.MathUtils.lerp(this.from.intensity, this.target.intensity, k);
    this.game.sun.position.lerpVectors(this.from.pos, this.target.pos, k);

    this.game.renderer.toneMappingExposure = THREE.MathUtils.lerp(this.from.exposure, this.target.exposure, k);
    if (this.game.world.stars) {
      this.game.world.stars.material.opacity = THREE.MathUtils.lerp(this.from.stars, this.target.stars, k);
    }
  }
  ...
  applyArea(id, immediate = false) {
    ...
    this.from = {
      ...
      hemiSky: this.game.ambient.color.clone(),
      hemiGround: this.game.ambient.groundColor.clone(),
      hemi: this.game.ambient.intensity,
      sun: this.game.sun.color.clone(),
      intensity: this.game.sun.intensity,
      pos: this.game.sun.position.clone(),
    ...
    if (immediate) {
      ...
      this.game.ambient.color.copy(this.target.hemiSky);
      this.game.ambient.groundColor.copy(this.target.hemiGround);
      this.game.ambient.intensity = p.hemi;
      this.game.sun.color.copy(this.target.sun);
      this.game.sun.intensity = p.intensity;
      this.game.sun.position.copy(this.target.pos);
